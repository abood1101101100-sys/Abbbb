...
@app.on_message(filters.command("iplookup", prefixes="/!"))
async def ip_lookup(_, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("عنوان IP مفقود.")
    ip_address = message.command[1]
    msg = await message.reply_text("جارٍ التحقق من عنوان IP...")
    try:
        res = await http.get(f"https://ipinfo.io/{ip_address}/json", timeout=5)
    except asyncio.TimeoutError:
        return await message.reply_text("انتهت مهلة الطلب.")
